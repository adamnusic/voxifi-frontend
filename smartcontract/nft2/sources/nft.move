module admin::voxify{
    ...
    const COLLECTION_NAME: vector<u8> = b"Voxify";
    const COLLECTION_DESCRIPTION: vector<u8> = b" A prediction market for voice verification. Like Polymarket for deepfake voices. Users hear a real voice and a deepfake voice and bid on which is real and which is fake. The idea is to gamify voice identification with financial incentives in order to build-up a verification dataset";

    const COLLECTION_URI: vector<u8> = b"https://i.nextmedia.com.au/News/voice_biometrics_voiceprint.jpg";

    // Token information
    const TOKEN_DESCRIPTION: vector<u8> = b"Voxify NFT";
    const TOKEN_URI: vector<u8> = b"www.abc.com";
